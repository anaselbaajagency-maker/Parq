...
interface DynamicListingInitialData {
    id?: number | string;
...
type ListingSubmitPayload = Record<string, string | number | string[]>;

export default function DynamicListingForm({ initialData, isEdit }: { initialData?: DynamicListingInitialData, isEdit?: boolean }) {
    const router = useRouter();
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState('');

    // Auth & Wallet Mock
    const [walletBalance] = useState(15);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const data: ListingSubmitPayload = {};
        ...
        if (categoryId) {
            data.category_id = parseInt(String(categoryId), 10);
        }
        ...
        try {
            ...
            if (isEdit) {
                ...
                await api.listings.update(String(initialData.id), data, token);
            } else {
