...
export default function CreateListingPage() {
    ...
    useEffect(() => {
        ...
        if (isAuthenticated && user && !user.email_verified_at) {
            alert(t('listings_page.verify_email_first') || 'Veuillez vérifier votre adresse email avant de créer une annonce.');
            router.push('/tableau-de-bord' as any);
            return;
        }
        ...
        Promise.all([
            ...
            if (walletData && walletData.balance <= 0) {
                alert(t('listings_page.zero_balance') || 'Votre solde est de 0 DH. Veuillez recharger votre portefeuille pour créer une annonce.');
            ...
            <div className={styles.card}>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                        ...
                        >
                            {heroPreview ? (
                                <div className={styles.imagePreviewContainer}>
                                    ...
                                    <button type="button" onClick={(e) => { e.preventDefault(); setHeroImage(null); setHeroPreview(null); }} className={styles.removeImageBtn}>
                                        <X size={16} />
                                    </button>
                                </div>
                            ) : (
                                <label className={styles.uploadLabel}>
                                    <UploadCloud size={24} />
                                    <span>Cliquez ou glissez-déposez pour télécharger l&apos;image principale</span>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <div
                            className={`${styles.imagesGrid} ${isDragging ? styles.dragging : ''}`}
                            onDragOver={handleDragOver}
                            onDragLeave={handleDragLeave}
                            onDrop={handleDrop}
                        >
                        ...
                        <p className={styles.helperText}>{selectedFiles.length}/5 photos supplémentaires (Glissez-déposez autorisé)</p>
                    </div>
